...
def test_use_all_repbase_no_tsd():
    ...
    for feature in features:
        ...
        config.use_kmers = feature[4]  # 使用k-mer特征
...
def test_kmer_size_combination():
    # 创建一个新的工作簿
    import openpyxl
    ...
    for i in range(max_num):
        internal_kmer_sizes = random.choice(internal_kmer_sizes_array)
        terminal_kmer_sizes = random.choice(terminal_kmer_sizes_array)
        config.internal_kmer_sizes = internal_kmer_sizes
        config.terminal_kmer_sizes = terminal_kmer_sizes

        print('internal_kmer_sizes:', internal_kmer_sizes)
        print('terminal_kmer_sizes:', terminal_kmer_sizes)
