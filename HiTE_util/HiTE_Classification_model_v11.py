# 融合了Seq, LTR, TIR, TSD, Domain等多种特征进行训练，同时包含k-mer的位置信息，将信息转为二维矩阵表示，并使用二维CNN
# 使用三个CNN模型进行连接，第一个模型用于处理Seq, LTR, TIR序列的k-mer特征，维度是4^k * p；
# 第二个模型用于处理TSD seq, TSD_len和Domain特征，维度是11*4+1+29;

# 只使用第一个模型，k=3, p=50
# Accuracy: 0.7477297008547008
# Precision: 0.7767418677560637
# Recall: 0.4909906968820124
# F1: 0.5351369432775798
# k=1,2,3,4. p=10 kernel_size: (1, 3)
# Accuracy: 0.7855235042735043
# Precision: 0.825500898889747
# Recall: 0.5545755320932796
# F1: 0.6195443435972083
# k=1,2,3,4. p=20 kernel_size: (1, 3)
# Accuracy: 0.7588141025641025
# Precision: 0.7578245583780374
# Recall: 0.46406038768042457
# F1: 0.5156371148419695
# k=1,2,3,4. p=1 kernel_size: (3, 1)
# Accuracy: 0.8548344017094017
# Precision: 0.9158408396113088
# Recall: 0.6899372967134135
# F1: 0.740682761163827
# k=1,2,3,4. p=10 kernel_size: (3, 3)
# Accuracy: 0.8333333333333334
# Precision: 0.8974254696883911
# Recall: 0.6645311963751181
# F1: 0.7250860477082446
# k=1,2,3,4,5. p=10 kernel_size: (3, 3)
# Accuracy: 0.8297275641025641
# Precision: 0.8864058238705272
# Recall: 0.6214620664867047
# F1: 0.6782836900396841
# k=1,2,3,4. p=20 kernel_size: (3, 3)
# Accuracy: 0.8100961538461539
# Precision: 0.9013275676523477
# Recall: 0.6022132228225284
# F1: 0.6647364685364829
# k=1,2,3,4. p=20. kernel_size: (3, 3)+4*Conv2
# Accuracy: 0.8293269230769231
# Precision: 0.8922875961825371
# Recall: 0.654306778611729
# F1: 0.7179955136950408
# k=1,2,3,4. p=20. kernel_size: (3, 3)+5*Conv2
# Accuracy: 0.8319978632478633
# Precision: 0.7788580386479792
# Recall: 0.6367226025981663
# F1: 0.679601834156092
# k=1,2,3,4. p=1. kernel_size: (3, 1)+5*Conv2
# Accuracy: 0.8561698717948718
# Precision: 0.9068582072402306
# Recall: 0.704155623075466
# F1: 0.7511016401239666

# 第二个模型：只使用k-mer位置信息，不使用频率

# 第一个模型+第二个模型 k=1,2,3,4. p=20. kernel_size: (3, 3)+4*Conv2
...
def get_compiled_model():
    ...
    # 2. CNN model
    model = Sequential()
    model.add(Conv2D(32, (3, 1), activation='relu', input_shape=(X_feature_len, p, 1)))
...
if __name__ == '__main__':
    ...
    # 训练和验证集
    X_train = X_train.reshape(X_train.shape[0], X_feature_len, p, 1)
    X_validate = X_validate.reshape(X_validate.shape[0], X_feature_len, p, 1)
    ...
    X_test = X_test.reshape(X_test.shape[0], X_feature_len, p, 1)
