# 我们发现加入k-mer的位置信息不会提升模型性能。
# 融合了Seq, LTR, TIR, TSD, Domain等多种特征进行训练
# 使用二个模型进行连接
# 第一个CNN模型用于处理Internal seq, LTR序列的k-mer特征，维度是4^k。
# 以及TSD seq, TSD_len, domain和序列的首尾共10bp特征，维度是11*4+1+29+10*4。
# 第二个Transformer模型用于处理TIR特征，最长序列维度是60。

# Accuracy: 0.925953375555855
# Precision: 0.9434754930380316
# Recall: 0.8431124216226898
# F1: 0.8728971350695063

# Accuracy: 0.937339981134618
# Precision: 0.9483715626284918
# Recall: 0.8740622861091789
# F1: 0.9054017761970407
...
def run_training(batch_size=32, epochs=1, use_checkpoint=1):
    ...
    model.fit([X_num_train, X_seq_train], Y_train_one_hot, validation_data=([X_num_validate, X_seq_validate], Y_validate_one_hot), callbacks=callbacks,
             batch_size=batch_size, epochs=epochs, verbose=1)
    # model.fit(X_num_train, Y_train_one_hot,
    #           validation_data=(X_num_validate, Y_validate_one_hot), callbacks=callbacks,
    #           batch_size=batch_size, epochs=epochs, verbose=1)
    return model
...
def hybrid_model(cnn_model, transformer_model):
    ...
    d_model = 16
    num_layers = 2
    transformer_input_shape = (max_seq_length,)

    cnn = cnn_model(cnn_input_shape)
    transformer = transformer_model(transformer_input_shape, vocab_size, num_heads, d_model, num_layers, max_seq_length)
...
#
#     cnn = cnn_model(cnn_input_shape)
#     #transformer = transformer_model(transformer_input_shape, vocab_size)
#
#     #combined_output = concatenate([cnn.output, transformer.output])
#     #dense_layer = Dense(128, activation='relu')(combined_output)
#     dense_layer = Dense(128, activation='relu')(cnn.output)
...
def get_compiled_model():
    ...
    # # 2. CNN model
    # model = Sequential()
    # model.add(Conv2D(32, (3, 1), activation='relu', input_shape=(X_feature_len, 1, 1)))
...
if __name__ == '__main__':
    ...
    domain_train_path = data_dir + '/repbase_train_part.64.ref.update.domain'
    domain_test_path = data_dir + '/repbase_test_part.64.ref.update.domain'

    # train_path = data_dir + '/test.ref.update'
    # test_path = data_dir + '/test.ref.update'

    # Step 1: 加载repbase数据。将序列划分成internal_seq, LTR, TIR三个部分，分别用k=1, 2, 3, 4频次表示internal_seq, LTR, TIR;TSD用one-hot编码表示，用多1位表示TSD_length;将domain转为One-hot编码
    parts = ['internal_seq', 'LTR']
    kmer_sizes = [1, 2, 3, 4]
    X_feature_len = 11 * 4 + 1 + 29 + 10 * 4
    for part in parts:
    ...
    divide_data_most_part = int(X_num.shape[0] * 0.8)
    X_num_train = np.array(X_num[0:divide_data_most_part])
    X_num_validate = np.array(X_num[divide_data_most_part:])
    X_seq_train = np.array(X_seq[0:divide_data_most_part])
    X_seq_validate = np.array(X_seq[divide_data_most_part:])
    Y_train = np.array(Y[0:divide_data_most_part])
    Y_validate = np.array(Y[divide_data_most_part:])

    # Step 4: 将数据reshape成模型接收的格式
    # 训练和验证集
    X_num_train = X_num_train.reshape(X_num_train.shape[0], X_feature_len, 1)
    X_num_validate = X_num_validate.reshape(X_num_validate.shape[0], X_feature_len, 1)
    X_num_train = X_num_train.astype('float64')
    X_num_validate = X_num_validate.astype('float64')
    X_seq_train = X_seq_train.astype('float64')
    X_seq_validate = X_seq_validate.astype('float64')
    ...
    X_num_test, X_seq_test, Y_test = generate_hybrid_feature_mats(X_test, Y_test, Y_test_name, all_wicker_class, kmer_sizes, threads)
    X_num_test = X_num_test.reshape(X_num_test.shape[0], X_feature_len, 1)
    X_num_test = X_num_test.astype('float64')
    X_seq_test = X_seq_test.astype('float64')

    Y_test_one_hot = np_utils.to_categorical(Y_test, int(class_num))
    Y_test_one_hot = np.array(Y_test_one_hot)

    print('X_num_train, X_seq_train and Y_train_one_hot shape: ')
    print(X_num_train.shape, X_seq_train.shape, Y_train_one_hot.shape)
    print('X_num_validate, X_seq_validate and Y_validate_one_hot shape: ')
    print(X_num_validate.shape, X_seq_validate.shape, Y_validate_one_hot.shape)
    print('X_num_test, X_seq_test and Y_test_one_hot shape: ')
    print(X_num_test.shape, X_seq_test.shape, Y_test_one_hot.shape)
    ...
    # Step 9: 评估模型
    loss, accuracy = model.evaluate([X_num_test, X_seq_test], Y_test_one_hot, batch_size=batch_size, verbose=1)
    #loss, accuracy = model.evaluate(X_num_test, Y_test_one_hot, batch_size=batch_size, verbose=1)
