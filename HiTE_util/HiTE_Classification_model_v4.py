#注：取最长的序列长度当做维度（~36k），利用CNN
#测试结果：Accuracy： ， Precision： ，Recall： ， F1：
...
#X -> {seq_name: [0, 4, 2, 3,...], }
X = generate_mats(X, seq_names, kmer_sizes, threads)
#print(X)
print(X.shape)
...
model.fit(X_train, Y_train_one_hot, validation_data=(X_validate, Y_validate_one_hot),
          batch_size=32, epochs=6, verbose=1)   ##epoch An epoch is an iteration over the entire x and y data provided
...
X_test = generate_mats(X_test, seq_names, kmer_sizes, threads)
