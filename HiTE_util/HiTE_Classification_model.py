...
from keras.layers import Dense, Dropout, Activation, Flatten, LSTM, Conv2D, MaxPooling2D, Bidirectional
...
if __name__ == '__main__':
    ...
    # Step 1: 加载repbase数据。将序列划分成seq, LTR, TIR，TSD四个部分，分别用k=5, 4, 3频次表示seq, LTR, TIR。TSD用one-hot编码表示，用多1位表示TSD_length
    kmer_sizes = [5, 5, 5]
    X_feature_len = 11 * 4 + 1
    for kmer_size in kmer_sizes:
        X_feature_len += pow(4, kmer_size)

    X, Y, seq_names = load_repbase_with_TSD(train_path)
    ...
    # 测试集
    X_test, Y_test, Y_test_name = load_repbase_with_TSD(test_path)
