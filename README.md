...
Therefore, it is recommended that the TE library to be classified consists of full-length TEs. For LTR retrotransposons, where many TE libraries often 
divide terminal and internal sequences, such as `Copia-62_PHord-LTR` and `Copia-62_PHord-I` in Repbase, we suggest restoring them to full-length LTRs before classification. 
...
For structurally incomplete fragmented TEs, you can use [RepeatClassifier](https://github.com/Dfam-consortium/RepeatModeler/blob/master/RepeatClassifier) configured with a [complete Dfam library](https://www.repeatmasker.org/RepeatMasker/) for classification using homology searches.
...
- [TE libraries generated by HiTE, EDTA, and RepeatModeler2](#libraries)
...
To reduce redundancy, many tools for TE identification separate the LTR termini and internal sequences of LTR retrotransposons in their output.
...
* For the output of EDTA, we recommend using the complete TE library, $genome.mod.EDTA.final/$genome.mod.EDTA.intact.fa.

* For the output of RepeatModeler2, we recommend using `NeuralTE/utils/reName_RM2.py` to rename the headers of the input TE library, formatted as `>Gypsy-171_OS-I` and `>Gypsy-171_OS-LTR`.
...
All experimental results can be reproduced step by step using xxx.
