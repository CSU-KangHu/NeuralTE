...
def get_both_ends_frame(candidate_sequence_path, ref_contigs, threads, temp_dir, output_dir, split_ref_dir):
    debug = 0
    flanking_len = 50
    starttime = time.time()
...
def get_left_frame(candidate_sequence_path, ref_contigs, threads, temp_dir, output_dir, split_ref_dir):
    debug = 0
    flanking_len = 50
    starttime = time.time()
    if os.path.exists(temp_dir):
...
if __name__ == '__main__':
    ...
    all_repbase_path = '/public/home/hpc194701009/left_LTR_real_dataset/all_ltr.ref'
    ...
    for species in species_genomes.keys():
        ...
        if os.path.exists(genome_path) and os.path.exists(cur_repbase_ltr):
            ...
            get_both_ends_frame(cur_repbase_ltr, ref_contigs, threads, temp_dir, output_dir, split_ref_dir)
